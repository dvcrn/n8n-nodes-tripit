...
export async function handleHotelOperation(
  ...
  if (operation === "addToTrip") {
    const params = {
      ...
      hotelName: this.getNodeParameter("displayName", 0) as string,
      checkInDate: this.getNodeParameter("startDate", 0) as string,
      checkOutDate: this.getNodeParameter("endDate", 0) as string,
      checkInTime: this.getNodeParameter("startTime", 0, "") as string,
      checkOutTime: this.getNodeParameter("endTime", 0, "") as string,
      timezone: this.getNodeParameter("timezone", 0) as string,
      street: this.getNodeParameter("address", 0) as string,
      ...
      numberGuests: parseInt(
        this.getNodeParameter("numberOfGuests", 0, "1") as string
      ),
      numberRooms: parseInt(
        this.getNodeParameter("numberOfRooms", 0, "1") as string
      ),
      bookingRate: this.getNodeParameter("roomRate", 0, "") as string,
      bookingDate: this.getNodeParameter("bookingDate", 0, "") as string,
      notes: this.getNodeParameter("notes", 0, "") as string,
      supplierConfNum: this.getNodeParameter(
        "supplierConfNum",
        0,
        ""
      ) as string,
      supplierContact: this.getNodeParameter(
        "supplierContact",
        0,
        ""
      ) as string,
      ...
      bookingSiteName: this.getNodeParameter(
        "bookingSiteName",
        0,
        ""
      ) as string,
      bookingSitePhone: this.getNodeParameter(
        "bookingSitePhone",
        0,
        ""
      ) as string,
      ...
      isPurchased: this.getNodeParameter("isPurchased", 0, false) as boolean,
    };
